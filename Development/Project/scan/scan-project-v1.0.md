...
6. **報告必寫入 `docs/Issues-Scan-Project.md`**;若檔案已存在,**先讀取比對舊內容**,最前方列出差異摘要(🆕 新增 / ✅ 已修 / ⏸ 未處理 / 🔄 異動),然後**合併寫入**:已修復項移除,未處理項保留,新增項加入
...
報告**必須寫入** `docs/Issues-Scan-Project.md`(專案根目錄下的 `docs/`)。

**持久化模型**:此檔是**累積式問題追蹤器**,不是每次掃描的快照。
- 首次掃描 → 建立檔案寫入全部發現
- 已存在 → 讀取比對後**合併寫入**:
  - ✅ **已修復**(舊有新無) → **從檔案中移除**
  - ⏸ **仍未處理**(兩次都有) → **保留原紀錄**(可以註記「首次發現:<日期>」以追蹤累積)
  - 🆕 **新增**(舊無新有) → **加入**
  - 🔄 **異動**(嚴重度 / 位置變化) → 更新內容

### 0. 差異摘要(僅當舊檔存在時出現,放最前)

比對規則:以 `R-xxx` / `AD-xxx` ID + 檔案路徑為 key 判定同一問題。

```markdown
## 🔄 與前次掃描差異(vs 2026-04-20 14:30)

- 🆕 新增:3 項
  - [R-ENV-001] `src/lib/db.ts:8` 機密寫死
  - ...
- ✅ 已修復:5 項(舊有新無)
  - [R-BE-009] CORS `*` — 改白名單
  - ...
- ⏸ 仍未處理:12 項
- 🔄 異動:2 項(嚴重度或位置變更)
  - [R-DB-002] 從 🟡 升為 🟠(多張表都缺 `updated_at`)
```

若首次掃描 / 無舊檔 → 跳過此區塊,直接進「1. 總覽」。
