...
- **`<script id="arch">`**:重畫主架構,**禁**保留模板裡 React/FastAPI/PG 的範例節點(那是 Template 預設,跟現在偵測結果不一定相同)。用偵測到的真實層級 / 元件 / 邊界 / Owner。**保留三色 `classDef`(user / it / shared / critical)**,新節點記得 `class X it` 之類正確染色
- **`<script id="flow-auth">`** 與 **`<script id="flow-deploy">`**:
  - 若偵測到 auth 機制 → 改寫 auth 流程(例:JWT cookie / SSO / API key)
  - 若偵測到 CI/CD → 改寫部署流程
  - 偵測不到 → **刪掉該整段 `<script>` 區塊**(別留 Template 預設範例混進來),picker 自動不會列
...
本 skill 內建的 `_template/` 是分發源,應與 Template 主庫 `Template/docs/Arch/diagrams/_template/` 內容一致(兩處同步),且格式對齊 `Architecture/<日期>-<...>-v*/diagram.html`(公司架構參考實作)。
