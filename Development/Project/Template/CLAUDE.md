...
- **本地開發優先**:`uvicorn` + frontend dev server;PostgreSQL 本機 install
...
| --- | --- | --- |
| `.env` | `.env.dev`(連 `localhost:5432`) | `.env.staging` / `.env.prod`(連實際 host,例 `db.example.com`) |
...
4. 本機 PostgreSQL 服務已啟動(預設 port `5432`)
