...
從「Harness-Engineering 在專案目錄裡」一路自動跑到「backend/ + frontend/ 可跑」的單入口 skill。LLM 負責:Harness-Engineering/ 自動展開 → 環境檢查 → 收參 + 校驗 → 呼叫 `scaffold.mjs` → 跑 acceptance。**不**自由發揮、**不**內嵌任何模板內容。
...
| 3 | 使用者 | 把 `Harness-Engineering/skills/init-project/` cp 到 `.claude/skills/init-project/`(專案 local 或 `~/.claude/skills/` 全域皆可) |
...
> 設計目的:讓使用者只要做 3 個手動動作(下載 Harness-Engineering / cp 到專案 / cp `skills/init-project` 到 `.claude/skills/`),其餘**全部由本 skill 接手**。
...
1. cp `Harness-Engineering/CLAUDE.md` → `<cwd>/CLAUDE.md`
2. cp `Harness-Engineering/AGENTS.md` → `<cwd>/AGENTS.md`
3. cp -r `Harness-Engineering/docs/Design-Base/` → `<cwd>/docs/Design-Base/`
4. cp -r `Harness-Engineering/prompts/` → `<cwd>/docs/Prompts/`(注意:目錄改名 `prompts` → `Prompts`)
5. 為 `<cwd>/docs/Prompts/*.md`(略過 `README.md`)產 `<cwd>/.claude/commands/<name>.md`,內容單行 `@docs/Prompts/<name>.md`
6. cp 其餘 `Harness-Engineering/skills/*`(除 `init-project` 之外的子目錄,例如 `start-dev` / `stop-dev`)→ `~/.claude/skills/<name>/`(全域,使用者其他專案也用得到)
7. **刪除整個 `<cwd>/Harness-Engineering/`**(✱ 不可逆)

選項(對齊心法 § 7,Recommended = 不看也不會出事的選項):

- **只 cp 不刪 Harness-Engineering/ (Recommended)** — 1–6 都做、跳過 7;Harness-Engineering/ 留在專案內,使用者之後想清再清(沒誤刪風險)
- **同意全部執行** — 走完整 1–7,含**不可逆刪除 Harness-Engineering/**(使用者讀過再選)
- **中止** — 什麼都不做,離開 skill

> 雖然 § 0d 對 Harness-Engineering/ 刪除還會獨立再確認一次,但 Recommended 仍走最安全路徑 — fail-safe 防呆。

> 不 cp 的東西:`Harness-Engineering/skills/init-project/`(使用者步驟 3 已自行裝過,不重蓋)、`Harness-Engineering/plan/`(模板自身演進史)、`Harness-Engineering/README.md`(模板索引,不是專案 README)。
...
依使用者選擇執行步驟 1–6;最後一步「刪除 `Harness-Engineering/`」**獨立再確認一次**(因不可逆),確認後才刪。刪除前先確認 cwd 不在 `Harness-Engineering/` 子樹內(否則 PowerShell 會拒絕)。
...
回報「Harness-Engineering/ 已展開並移除,規範檔 + 額外 skills(start-dev / stop-dev / ...)已就位」,接著進環境檢查。
...
target 目錄允許三種狀態:
1. **空**(或僅含 `.git/`)→ 直接落檔
2. **僅含 Harness-Engineering 規範檔**(`CLAUDE.md` / `AGENTS.md` / `docs/Design-Base/` / `docs/Prompts/` / `.claude/commands/`)→ 這是「先 cp Harness-Engineering 規範,再跑 /init-project」的合法 pre-state,**加 `--force`** 後落檔(scaffold 不會動到這些規範檔)
...
- **禁止**自行**撰寫**規範檔內容(CLAUDE.md / AGENTS.md / docs/Design-Base / docs/Prompts)— § 0 只 cp 既有 `Harness-Engineering/` 內檔案,不可生成新文字
...
| 職責 | 由誰處理 |
| --- | --- |
| Harness-Engineering/ 偵測 + 規範檔 cp + 自我刪除(§ 0) | LLM(本 SKILL.md) |
