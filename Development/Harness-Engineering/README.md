...
- user 要求 fork / 套用此 Harness-Engineering 至新專案 → 依〈使用協議〉情境 A(3 個手動動作 + 跑 `/init-project`)
- user 跑 `/init-project` skill → 自動處理 Harness-Engineering/ 展開 + 工具檢查 + scaffold,使用者只要回答 `AskUserQuestion`
- 需查詢 Harness-Engineering 內檔案位置 / 用途 → 看〈內容〉
...
### 不適用(若 user 提及下列,告知此 Harness-Engineering 不適用)
...
│   ├── sso-init/            接公司 DF-SSO 中央登入器(dispatcher → FastAPI / Next.js / Spring 變體 + 隨附 INTEGRATION.md)
│   ├── commit-all/          一鍵提交並推送當前分支所有變更((AI) 前綴)
│   ├── merge-main/          合併當前分支到 main 並推送
...
### 情境 A:user 從零套用 Harness-Engineering 到新專案(統一入口)

整體流程 = **使用者 3 個手動動作 + 1 個自動觸發**,其餘全由 `/init-project` skill 接手:

| # | 誰 | 動作 |
| --- | --- | --- |
| 1 | 使用者 | 下載 Harness-Engineering 整個資料夾 |
| 2 | 使用者 | 把 Harness-Engineering 整個 cp 進專案目錄(結果是 `<project>/Harness-Engineering/...`) |
| 3 | 使用者 | 把 `<project>/Harness-Engineering/skills/init-project/` cp 到 `~/.claude/skills/init-project/`(全域,推薦)或 `<project>/.claude/skills/init-project/`(專案 local)|
| 4 | 使用者 | 在專案根目錄跑 `/init-project` |
| 4a | skill | 偵測 `<project>/Harness-Engineering/` → 用 `AskUserQuestion` 徵詢同意 → cp `CLAUDE.md` / `AGENTS.md` / `docs/Design-Base/` / `prompts/`(改名 `docs/Prompts/`)→ 產 `.claude/commands/<name>.md` → cp `Harness-Engineering/skills/*`(除 `init-project`,如 `start-dev` / `stop-dev`)到 `~/.claude/skills/` → **刪除整個 `<project>/Harness-Engineering/`** |
...
### 情境 B:user 在已套過 Harness-Engineering 的專案重跑 `/init-project`

若 `<project>/Harness-Engineering/` 已不在(被前次 § 4a 刪了),`/init-project` 跳過 § 0 直接進環境檢查 + scaffold。**禁**重複 scaffold 既存 `backend/` / `frontend/`(scaffold.mjs 預檢會擋,除非 `--force`)。

### 情境 C:user 跑 `/scan-project`

依 `prompts/scan-project.md` 執行(掃描 + 累積式報告寫入 `docs/Tasks/scan-project/`)。

### 情境 D:user 詢問 Harness-Engineering 規範細節 / 衝突解決
