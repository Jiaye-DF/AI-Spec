...
```
<project>/
├── AGENTS.md / CLAUDE.md        # 從 Harness-Engineering 複製
├── docs/
...
> 寫進 `Harness-Engineering/AGENTS.md`,讓 agent 依子任務情境精準載入,不讀無關規則。
...
- `Harness-Engineering/AGENTS.md` — 跨工具事實層,六區塊 + Capabilities Baseline + **Just-in-time Loading 對照表(完整版)** + Rule Precedence
- `Harness-Engineering/CLAUDE.md` — Claude 特化薄層 ~35 行,import `@AGENTS.md`,內含 Plan mode / Skill / Sub-agent / TodoWrite / Verify loop
- `Harness-Engineering/prompts/` — README + init-project + propose-to-tasks + scan-project + reflect-rules
...
- 重寫 `Harness-Engineering/AGENTS.md` 為事實層 + **完整 Just-in-time Loading 對照表**(Phase 1 最重要產出)
- 重寫 `Harness-Engineering/CLAUDE.md` 為 Claude 特化薄層
- 建 `Harness-Engineering/prompts/` 五檔(README + init-project + propose-to-tasks + scan-project + reflect-rules)
- 建 `Harness-Engineering/docs/Design-Base/` 9 個資料夾骨架,每個資料夾先填 `00-overview.md`(入口 + 該 area 風格地板)
- 更新 `Harness-Engineering/README.md`
...
## 升級協議(Harness-Engineering 自身演進)

- Harness-Engineering 版本演進以 git tag 標記(`harness-engineering-v1.0` / `harness-engineering-v1.1`)
