...
8. **一次性任務 container 必須設 `restart: "no"`** — 只跑一次就 `exit` 的 migration / seeding container，避免被重新拉起重複執行（見「restart 重啟策略」章節）
...
- Coolify 真正展開的 magic env 僅限 `SERVICE_URL_*` / `SERVICE_FQDN_*` / `SERVICE_PASSWORD_*` / `COMPOSE_PROJECT_NAME`（注意：`SERVICE_PASSWORD_*` 由 Coolify 自動生成，但**不能**用 `${...}` 引用進其他變數的值）；其他變數（如 `SEQ_FIRSTRUN_ADMINPASSWORD`）請於 Coolify Env Vars 面板填入，compose 寫成冒號後留空即可
...
services:
  ...
  seq:
    image: datalust/seq:2025.2   # 綁定版本，勿用 latest
    restart: unless-stopped
    environment:
      ACCEPT_EULA: "Y"
      SERVICE_URL_SEQ_80:
...
container 依生命週期分兩類，重啟策略不同。**錯設一次性 container 的 restart 策略會造成 migration 重複執行**，必須依下表處理。

### 長駐服務 vs 一次性任務

| 類型 | 範例 | restart 策略 |
| --- | --- | --- |
| **長駐服務**（long-running） | frontend / backend / postgres / redis / seq / adminer | `restart: unless-stopped`（或省略，沿用既有寫法） |
| **一次性任務**（run-once） | DB migration / seeding / schema 初始化 | **`restart: "no"`（必填）** |

### 為什麼一次性 container 必須設 `restart: "no"`

一次性 migration container 跑完 migration 後會 `exit 0`。若**沒設** restart 或設成 `always` / `unless-stopped`：

- container `exit` 後會被 Docker / Coolify 重新拉起 → migration **重複執行**
- 可能造成重複 seed 資料、schema 套用衝突、container 無限重啟 loop
- Coolify 部署畫面會因為這個 container 持續 restart 而顯示異常狀態，遮蓋真正的部署結果

`restart: "no"` 明確宣告「這個 container 跑完就停，不要重啟」，是 migration container 的正確終態。

### YAML 寫法：`"no"` 必須加引號

YAML 1.1 會把裸寫的 `no` 解析成 boolean `false`。雖然 Docker Compose 多數情況能容錯，但本規範一律加引號保留字串，與 `ACCEPT_EULA: "Y"` 的處理一致：

```yaml
# ✅ 正確
restart: "no"

# ❌ 錯誤：裸 no 會被 YAML 解析成 false
restart: no
```

### 一次性 migration container 寫法
...
```yaml
  ...
  backend:
    # …其餘設定見主範例，此處僅示範與 migrate 的相依…
    depends_on:
      migrate:
        condition: service_completed_successfully   # 等 migrate exit 0 再啟動
```

說明：

- migration container 通常與 backend **共用同一個 image**（`build.context` 指向相同目錄），只是 `command` 換成 migration 指令
- `command` 內**不要帶 `${Variables}`**（核心規則 3），變數一律放 `environment`
- migration container 屬 **app-level service**，不適用選配服務（adminer / seq）的雙向校驗規則
- `depends_on` 只保證 postgres **容器已啟動**，不保證 DB **已可接受連線**。若 migration 偶發連不到 DB，請在 migration script 內加連線重試，或對 postgres 設 healthcheck 後改用 `depends_on: { postgres: { condition: service_healthy } }`
- 依賴 migration 結果的長駐服務（如 backend）必須用 `depends_on: { migrate: { condition: service_completed_successfully } }` 等 migrate `exit 0` 後再啟動，否則會與 migration 競速、在 schema 未套用時就啟動報錯
- migrate container 跑完會顯示 `exited (0)`，屬正常終態；判斷部署是否成功以長駐服務的健康狀態與 Deployment Log 為準，勿被 migrate 的 exited 狀態誤導
- 長駐服務（frontend / backend / postgres…）**不要**套用 `restart: "no"` — 它們崩潰時需要自動拉起
...
| 一次性 migration container 未設 `restart: "no"` | 疑似一次性任務 container（service 名含 `migrate` / `seed` / `init` 或 `command` 為 migration 指令）建議加 `restart: "no"`，避免重複執行 |
...
規則僅針對選配服務（`adminer` / `seq`）；**app-level service**（frontend / backend / 業務 container）不受反向規則限制。一次性 migration container 屬 app-level service，`restart: "no"` 的檢查為 warning 級（無法 100% 確定某 container 是否一次性，不擋部署）。
...
7. 若有一次性 migration container，於 Deployment Log 確認其 `exit 0` 後**不再被重啟**（若一直 restart 代表漏設 `restart: "no"`）
...
| migration 重複執行 / 一次性 container 一直 restart | 一次性任務 container 沒設 `restart: "no"`（或設成 `always` / `unless-stopped`） | 一次性 container 加 `restart: "no"`（`"no"` 加引號） |
